    ...
    def search_corpus(self, corpus_name, conn):
        try:
            if corpus_name == "":
                ...
                cursor = conn.cursor(cursor_factory=RealDictCursor)
                query=(f"SELECT corpus_id, corpus_name, corpus_type, language, source_type,flag, "
                       f"lastupdated_ts, (SELECT teamname FROM cfg_udops_teams_metadata tm WHERE tm.team_id IN (SELECT team_id FROM cfg_udops_teams_acl cta WHERE cta.corpus_id = corpus_metadata.corpus_id ) ) AS team_name FROM corpus_metadata"
