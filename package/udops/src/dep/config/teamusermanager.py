...
class teamusermanager:
    ...
    def get_s3_path(self):
        ...
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute('select team_id from cfg_udops_teams_metadata where teamname = "{team}";')
        git_token = config.get('github','access_token')
        url = 'https://api.github.com/user'
        headers = {'Authorization': f'token {git_token}'}
        response = requests.get(url, headers=headers)
        github_username = response.json()['login']
        cursor.execute('select user_id from cfg_udops_users where user_name = "{github_username}" and teamname = "{team}";')
